        ...
        {filtered ? (
          <div className="px-3 py-2">
            {filtered.length > 0 ? filtered.map(item => (
              <SidebarItem key={item.path} item={item} isActive={isActive(item.path)} onClick={handleItemClick} />
            )) : (
  ...
  return (
    ...
    >
      <div className="w-full px-4 h-14 flex items-center gap-3">
        ...
        <button
          className="md:hidden block p-2 text-cyber-muted hover:text-white transition-colors"
          onClick={onMenuClick}
